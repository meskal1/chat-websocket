import { PropsWithChildren, useCallback, memo, FC } from 'react'

import { ChannelListMessengerProps } from 'stream-chat-react'

import { useWorkspaceController, Workspace } from '../../../../../context/WorkspaceController'
import './SidebarChannelList.scss'

export type SidebarChannelListProps = ChannelListMessengerProps & {
  type: string
}

export const SidebarChannelList: FC<PropsWithChildren<SidebarChannelListProps>> = memo(
  ({ children, error = false, loading, type, ...restProps }) => {
    const { displayWorkspace } = useWorkspaceController()

    const handleAddChannelClick = useCallback(() => {
      displayWorkspace(`Admin-Admin-Channel-Create__${type}` as Workspace)
    }, [type, displayWorkspace])

    if (error) {
      return type === 'team' ? (
        <div className="team-channel-list">
          <p className="team-channel-list__message">
            Connection error, please wait a moment and try again.
          </p>
        </div>
      ) : null
    }

    if (loading) {
      return (
        <div className="team-channel-list">
          <p className="team-channel-list__message loading">
            {type === 'team' ? 'Channels' : 'Messages'} loading....
          </p>
        </div>
      )
    }

    return (
      <div className="team-channel-list">
        <div className="team-channel-list__header">
          <p className="team-channel-list__header__title">
            {type === 'team' ? 'Channels' : 'Direct Messages'}
          </p>
          <button className="team-channel-button" onClick={handleAddChannelClick} />
        </div>
        {children}
      </div>
    )
  }
)
